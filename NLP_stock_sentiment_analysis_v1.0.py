...
def analyze_stock_sentiment(url, stock_symbol):
    ...
    stock_long_name_truncated = stock_long_name.replace("The", "").replace("Inc.", "").replace("Corporation", "").strip()
    stock_short_name = stock_long_name_truncated.split(" ")[0].split(",")[0].split(".")[0]
    entities = [stock_long_name, stock_long_name_truncated, stock_short_name, stock_symbol]
    ...
    text_snippets = [sent.text for sent in doc.sents if any(entity in sent.text for entity in entities)]
    ...
    sentiment_score = (pos_count / total_count) - (neg_count / total_count) if total_count > 0 else 0

    return sentiment_score

# Example usage
url = 'https://finance.yahoo.com/news/why-burlington-stores-stock-jumped-221554778.html'  # Replace with actual URL
stock_symbol = 'BURL'
